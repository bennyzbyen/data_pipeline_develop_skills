---
name: report-codegen
description: Generate or review portable Python DataEngine report development project code from AI-readable development docs using DataSource/DataProcess/DataStorage structure. Uses bundled supervisor-portal-style patterns and templates first, with datahub_supervisor_portal and vehicle_verification_202605221431 only as optional extra references when available.
---
...
- Primary source of truth: bundled `references/supervisor_portal_patterns.md`, `references/vehicle_verification_patterns.md`, and `assets/minimal_report_project/`.
...
- Credentials, IPs, tokens, app keys, and app secrets must remain placeholders.
...
2. Read `references/vehicle_verification_patterns.md` when the report uses HBase + FS + ClickHouse, vehicle/order logic, or multiple summary outputs.
3. If `structured_facts.json` exists, load it before writing code and use `report_sources`, `report_clickhouse_targets`, `report_targets`, `report_field_mappings`, and `report_schedules`.
...
4. Use `assets/minimal_report_project/` as the fallback scaffold if no local sample is available.
5. If optional samples are available, inspect them as extra style guidance; never require them.
6. Inspect the target directory and any existing files.
7. State planned file modifications, why they are needed, and risks.
8. Generate only the modules needed for the current report.
9. Preserve project-local style and avoid unrelated refactors.
...
- for HBase prepare-data components: same period derivation, source HBase table, exported columns, SubSegment/filter rules, FS file naming, optional downstream pipeline activation, and placeholder-only credentials
