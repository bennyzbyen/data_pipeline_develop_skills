---
name: report-codegen
description: Use when generating, reviewing, or modifying portable Python DataEngine/DataHub report project code from AI-readable development docs, including KPI calculation, summary/detail outputs, ClickHouse writes, HBase/FS/MSSQL sources, or HBase prepare pipelines.
---
...
- Primary source of truth: bundled `references/supervisor_portal_patterns.md`, `references/vehicle_verification_patterns.md`, `references/executing_king_patterns.md`, `references/platform_client_usage.md`, and `assets/minimal_report_project/`.
...
4. If `structured_facts.json` exists, load it before writing code and use `report_sources`, `report_physical_targets` / `report_clickhouse_targets`, `report_targets`, `report_field_mappings`, `report_schedules`, `component_hints`, and `handoff_readiness`.
   ...
   - If `component_kind = executing_king_bysku_pipeline` or the plan mentions 执行为王 + bySKU + 新品/B5, read `references/executing_king_patterns.md`, inspect `prod_code_sample/datahub_executing_king` when available, and treat the work as a multi-component pipeline rather than a normal `standard_report`.
   ...
   - For Executing King bySKU plans, generate or modify one component at a time (`prepare_data`, `cal_npd`, or `cal_b5`) unless the user explicitly asks for the full chain. Do not claim a generic scaffold is complete until FS file selection, SKU params, detail/summary/ttl outputs, ClickHouse delete/insert, and R13P cleanup are implemented or verified.
...
- Do not flatten Executing King bySKU work into one generic report module. Preserve the prepare-data FS handoff and the separate NPD/B5 calculation components unless the existing project deliberately combines them.
