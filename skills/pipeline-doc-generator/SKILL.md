...
HTML separates the document-title link from the chapter outline. Keep the sidebar toggle in a dedicated narrow left rail that reserves its own layout space in both states, never as a floating button over the text. Keep it reachable while scrolling, expand the desktop content area when the directory is closed, and wrap long titles/paths without clipping. Use the keyboard-accessible native toggle with a compact icon, a light neutral sidebar, and reserved space outside the reading area; never use a dark full-height rail or vertical toggle text. The bundled `assets/document-navigation.js` tracks the current heading during scrolling, sets `aria-current="location"`, and keeps the active directory item visible. Both scripts must remain offline, fixed assets validated byte-for-byte and authorized by CSP hashes. Hide navigation controls when printing.
